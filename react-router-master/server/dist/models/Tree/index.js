...
class TreeBuild {
    constructor() {
        ...
        this.createHierarchy = {
            group: this.allJSONSdata.group.reduce((prev, { id, groupName: name, type = 'group' }) => (Object.assign(prev, { [id]: { type, id, name } })), {}),
            user: this.allJSONSdata.user.reduce((prev, { id, username: name, type = 'user' }) => (Object.assign(prev, { [id]: { type, id, name } })), {}),
            groups: {
                groups: Object.assign(this.allJSONSdata.GroupsToGroups.reduce((prev, group) => Object.assign(prev, group), {}), { root: this.allJSONSdata.group.filter(({ groupName }) => groupName === 'ROOT').map(({ id }) => id) }),
