...
class TreeBuild {
    ...
    constructor() {
        ...
        this.createHierarchy = {
            group: this.data.group.reduce((prev, {id, groupName: name, type = 'group'}) =>
                (Object.assign(prev, {[id]: {type, id, name}})), {}),
            user: this.data.user.reduce((prev, o) => (Object.assign(prev, {[o.id]: o})), {}),
            groups: {
                groups: Object.assign(
                    this.data.GroupsToGroups.reduce((prev, group) => Object.assign(prev, group), {}),
                    {root: this.data.group.filter(({groupName}) => groupName === 'ROOT').map(({id}) => id)}
                ),
